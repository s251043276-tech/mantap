...
# R code for Xbar.one Control Chart for Machine 2, 338K, 200kPa
library(tidyverse)
library(plotly)
library(qcc)

df_machine2_filtered <- X029...029 %>% filter(Machine == 2, Temperature == 338, Pressure == 200)
...
p_machine2 <- ggplot(plot_data_machine2, aes(x = Observation, y = PartLength)) +
  ...
  labs(title = "Xbar.one Chart for Part Length (Machine 2, 338K, 200kPa)",
       y = "Part Length",
       x = "Observation") +
  theme_minimal() +
